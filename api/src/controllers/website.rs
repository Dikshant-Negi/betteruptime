...
#[handler]
pub async fn create_website(Json(body):Json<WebsiteInput>,data:Data<&Arc<Mutex<AppState>>>)->Result<Json<WebsiteOutput>,Error>{
    if body.url.is_empty() || body.name.is_empty() || body.interval <=0 {
        return Err(Error::from_status(StatusCode::BAD_REQUEST));
    }

    let mut lock = data.lock().await;
    let res = lock.db.create_website(body.url,String::from("123456"),body.name,body.interval).await;
