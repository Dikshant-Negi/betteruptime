...
pub async fn  create_user(Json(body):Json<user_response::CreateUserInput>,data:Data<&Arc<Mutex<Store>>>)->Result<Json<user_response::CreateUserOuptput>,Error>{
    ...
    match res{
        Ok(_)=>{
           Ok( Json(user_response::CreateUserOuptput { success: (true), jwt: String::from("123456"), message:  String::from("User created successfully") }))
        }
        Err(_)=>{
...
#[handler]
pub async fn sigin(Json(body):Json<user_response::SignInput>,data:Data<&Arc<Mutex<Store>>>)->Result<Json<user_response::CreateUserOuptput>,Error>{
